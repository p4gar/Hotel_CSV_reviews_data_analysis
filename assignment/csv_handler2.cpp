...
double csvHandler::countSentimentWordsusingLinearSearch(const string &review)
{
    ...
    while (ss >> word)
    {
        ...
        else if (linearSearch(cleanedWord, negativeWords, negativeCount)) // Changed from `positiveCount` to `negativeCount`
        {
            negativeWordCount++;
            totalNegativeWords++;
...
void csvHandler::searchRecordByIndex(const string &filename, int index, csvHandler &csvHandlerObj)
{
    ifstream file(filename);
    if (!file.is_open())
    {
